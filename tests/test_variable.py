...
def test_variable_scale_binned():
    ...
    for factor in [random.uniform(0,10000) for x in range(100)]:
        ...
        assert( all( tuple_compare(x,y) for x,y in zip(testvar.values,scaled_values)) )

        # Check that inverse also works
        testvar.scale_values( 1. / factor)
        assert( all( tuple_compare(x,y) for x,y in zip(testvar.values,values)) )
...
def test_variable_scale_uncertainty():
    ...
    for factor in [random.uniform(0,10000) for x in range(100)]:
        ...
        assert( all(float_compare(x,y) for x, y in zip(testvar.values, scaled_values)) )
        assert( all(float_compare(x,y) for x, y in zip(testunc.values, scaled_uncertainty)) )
        

        # Check that inverse also works
        testvar.scale_values( 1. / factor)
        assert( all( float_compare(x,y) for x,y in zip(testvar.values,values)) )
        assert( all(float_compare(x,y) for x, y in zip(testunc.values, uncertainty)) )
